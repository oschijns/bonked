...
type ProcessKinematics<'q> = PreparedQuery<(
    &'q Collider,
    &'q Position,
    &'q Velocity,
    &'q BoundingBox,
    &'q mut CollisionStatus,
)>;
...
pub struct Querier<'q> {
    ...
    compute_static_boxes:
        PreparedQuery<Without<(&'q Collider, &'q Position, &'q mut BoundingBox), &'q Velocity>>,
    ...
    get_statics:
        PreparedQuery<Without<(&'q Collider, &'q Position, &'q BoundingBox), &'q Velocity>>,

    /// Process moving objects
    process_kinematics1: ProcessKinematics<'q>,

    /// Process moving objects
    process_kinematics2: ProcessKinematics<'q>,
            ...
            for (id2, (coll2, pos2, box2)) in self.get_statics.query(world).iter() {
                ...
                if id1 != id2 && box1.0.intersects(&box2.0) {
                    match contact(
                        ...
                        Ok(Some(contact)) => {
                            stat1.0.add_contact(&contact.point1, &contact.normal1);
                        }
                        Ok(None) => {}
                        Err(unsupported) => {
                            panic!["{}", unsupported];
                ...
                self.process_kinematics2.query(world).iter().skip(count)
            {
                ...
                if id1 != id2 && box1.0.intersects(&box2.0) {
                    match contact(
                        ...
                        Ok(Some(contact)) => {
                            stat1.0.add_contact(&contact.point1, &contact.normal1);
                            stat1.0.add_velocity(&vel2.0);
                            stat2.0.add_contact(&contact.point2, &contact.normal2);
                            stat2.0.add_velocity(&vel1.0);
                        }
                        Ok(None) => {}
                        Err(unsupported) => {
                            panic!["{}", unsupported];
